...
function parseSpokenLineHints(userText: string): SpokenLineHint[] {
  ...
  for (const part of parts) {
    ...
    /** 批发默认：单价*N斤（论斤时单价通常较小，如 1.2*36斤 → 单价1.2、36斤） */
...
- **「N*M斤」或「N×M斤」**：默认 **单价×数量** —— 单价 **N**（元/斤，一般较小）、数量 **M斤**（如 1.2*36斤 → 单价「1.2」、数量「36斤」、金额 43.2）；**禁止**把 N×M 合并成一个数量，也**禁止**把两数对调。
...
- 支持：今天、昨天、前天；**3月5日、三月五日**（用户**未说年份**时，一律用**当前公历年**的该月日，勿填往年）；**2025年3月5日**（说了四位数年份才用该年）；仅说「5号」为**今年**当月5日。
- **禁止未来日期**：记账日期不得超过今天；用户说「明天」或未说年份的月日若落在今天之后，按**今天**处理。
- **「几排几号」是摊位/购买方标识（如 6排7号），不是月日**；不要把「6排7号」解析成 6月7日，此类内容只写入购买方字段，不要输出「记账日期」。
...
「包+元」与「单价*数量斤」示例（用户：圆紫薯1包50元，圆紫薯1.2*36斤，孙悟空买的）：
{
  "商品明细": [
    { "商品": "圆紫薯", "数量": "1包", "金额": "50" },
    { "商品": "圆紫薯", "数量": "36斤", "单价": "1.2" }
  ],
  "${buyerLabel}": "孙悟空"
}
